...
Die Dependability-Taxonomie unterscheidet eine Kette aus Fault, Error und Failure \[1, S. 13 und 20–22\]. Ein Fault ist die Ursache, etwa eine getrennte SDA-Leitung oder ein blockierter Prozess. Der Fault kann einen fehlerhaften internen Zustand erzeugen, beispielsweise ein I2C-NACK oder einen veralteten JSON-Zustand. Wird die geforderte Funktion an der Systemgrenze nicht mehr erbracht, liegt ein Failure vor. Diese Trennung ist für die Evaluation wesentlich. Das Abziehen des GPS-Empfängers ist die Fehlerinjektion. have\_fix=false ist ein interner Fehlerzustand. Die Übertragung von GPS=ERR;MODE=LAST ist dagegen eine kontrollierte Reaktion, die einen unmarkierten Ausfall der Gesamtfunktion verhindert.

Unterspannung, Brown-out und intermittierende Versorgung. Ein Brown-out ist kein zwingend vollständiger Spannungsverlust, sondern ein Betriebszustand, in dem die Versorgung unter den für zuverlässige Ausführung erforderlichen Bereich absinkt. Balsamo et al. unterscheiden bei intermittierender Versorgung eine minimale Betriebsspannung sowie getrennte Schwellen für Hibernation und Wiederaufnahme; unterhalb der minimalen Spannung kann ein Prozessor unvorhersehbar arbeiten oder vollständig abschalten \[26, S. 15–16\]. Ein beobachteter Neustart bei einer am Netzteil eingestellten Spannung ist deshalb nicht automatisch mit der internen BOR-Schwelle des Mikrocontrollers gleichzusetzen.

Forschung zu intermittierend versorgten eingebetteten Systemen zeigt außerdem, dass wiederholte Spannungsausfälle nicht nur Verfügbarkeit, sondern auch Zustandskonsistenz betreffen. Mementos misst die verfügbare Energie und sichert den Ausführungszustand vor einem erwarteten Spannungsverlust in nichtflüchtigem Speicher \[27, S. 159–163\]. DINO zeigt, dass Neustarts an beliebigen Programmpunkten zu teilweise oder wiederholt ausgeführten Operationen und damit zu inkonsistenten Zuständen führen können \[28, S. 575–577\]. Die vorliegende Arbeit implementiert kein solches Checkpointing; die Arbeiten dienen als theoretische Grundlage für die Trennung von Spannungserkennung, definiertem Zustand, Neustart und anschließender Recovery.
...
#math.equation(block: true, numbering: (..) => "(2.1)", $T_"WDT" >= T_("max, normal") + T_"Jitter" + T_"Reserve"$)

Die Beziehung ist keine Normformel und kein formaler WCET-Nachweis, sondern beschreibt die allgemeine Dimensionierungslogik dieser Arbeit. Ein langer Timeout reduziert Fehlalarme, verlängert aber die Reaktion auf echte Hänger; ein kurzer Timeout reagiert schneller, kann bei hoher Last oder blockierendem I/O unnötige Neustarts auslösen. Die konkreten Timeout-Werte des aufgebauten Systems werden erst in Kapitel 4 festgelegt.
...
UART liefert einen Bytestrom ohne inhärente Rahmen- oder Integritätsgarantie. Ein Anwendungsprotokoll benötigt deshalb eine Rahmengrenze, eine maximale Länge und eine Prüfinformation. Die Arbeit verwendet eine ASCII-Zeile mit XOR-Prüfsumme. XOR ist einfach und schnell, erkennt aber deutlich weniger Fehlermuster als gut ausgewählte CRC-Verfahren \[12, S. 59–60 und 67–69, 13, S. 145–146\].

// Vorlage: PDF-Seite 18
== I2C und lokale Bus-Recovery <sec-2-11>

I2C verwendet Open-Drain-Leitungen und adressierte Transfers. Die offizielle Spezifikation beschreibt unter anderem START/STOP-Bedingungen, Acknowledgements und das Bus- Clear-Verfahren \[14, Abschn. 3.1.1, 3.1.4-3.1.6, 3.1.16\]. Bleibt eine Peripherie oder Leitung in einem fehlerhaften Zustand, darf eine blockierende BUSY-Prüfung die Anwendung nicht dauerhaft anhalten. Die Arbeit begrenzt Wartezeiten, setzt den I2C-Block zurück und erzeugt bei Bedarf Taktpulse zur Busfreigabe, bevor das LCD neu initialisiert wird. Das angeschlossene LCD2004-Zeichenmodul wird über den Adapter als HD44780-basiertes Display angesteuert \[19, S. 4\]. Der eingesetzte AZDelivery-I2C-Adapter verwendet einen PCF8574-I/O-Expander \[19, S. 4\]. Die Eigenschaften und der hardwareseitig einstellbare Adressbereich des Expanders sind im NXP-Datenblatt dokumentiert \[15, S. 1 und 5-8\].

// Vorlage: PDF-Seite 18
== GPS, NMEA und externe Wetterdaten <sec-2-12>

Als vollständiges USB-GPS-Gerät wurde ein unter der Vertriebsbezeichnung Binghe VK-162 erworbenes Modul eingesetzt. Das Gerät erschien unter Linux als /dev/ttyACM0. Seine interne Empfängereinheit identifizierte sich in den protokollierten USB- und Startmeldungen als Hardware der u-blox-7-Familie beziehungsweise als UBX-G70xx; diese Bezeichnung beschreibt die intern erkannte Einheit und nicht die Vertriebsbezeichnung des vollständigen USB-Geräts. Die experimentelle Identifikation ist in Anhang A dokumentiert. Die interne Empfängereinheit liefert NMEA-Datensätze über USB. Der RMC-Satz enthält Zeit, Status, Position und Bewegungsinformationen; nur der Status A kennzeichnet gültige Navigationsdaten \[16, S. 62\]. Die Umrechnung von Grad und Minuten in Dezimalgrad erfolgt nach

#math.equation(block: true, numbering: (..) => "(2.2)", $phi_"dez" = phi_"Grad" + frac(phi_"Minuten", 60)$)

Der Wetterdienst wird über eine HTTPS-Schnittstelle mit JSON-Antwort angesprochen \[17, Abschn. "Weather Forecast API"\]. Als externe Abhängigkeit ist er bewusst Teil des Fehlermodells: Netzwerkkonnektivität, DNS/TLS und die Verfügbarkeit beziehungsweise Antwort des Wetterdienstes können unabhängig voneinander ausfallen.
