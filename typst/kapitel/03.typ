...
Die Gesamtarchitektur folgt dem Datenpfad GPS -\> Wetterabfrage -\> UART -\> LCD (Abbildung 3.1). Der Raspberry Pi übernimmt GPS-Verarbeitung, externe Kommunikation und Prozessüberwachung; der STM32 übernimmt UART-Validierung, Anzeige und hardwarenahe Überwachung. Die UART-Verbindung bildet die Plattformgrenze. Die folgenden Abschnitte beschreiben die Ebenen in derselben Reihenfolge wie der Datenpfad; Komponentengrenzen werden dabei zugleich als Fehler- und Recovery-Grenzen betrachtet.
...
#vorlage("abb-3-3.png", "Abbildung 3.3: Eskalationshierarchie der Recovery-Strategien", art: "image")
...
Der globale Systemzustand unterscheidet zwischen stabilen Betriebszuständen und transienten Recovery-Zuständen. Der in Abbildung 3.4 dargestellte Automat ist eine systemweite Abstraktion der verteilten Implementierung. Er ist nicht als zusätzliche monolithische Softwarekomponente zu verstehen: Auf dem STM32 werden BOOT, NORMAL und DE- GRADED durch Bootdiagnose und Supervisor abgebildet; auf dem Raspberry Pi setzen die systemd-Zustände, Restart-Policies und Service-Watchdogs die Übergänge für einzelne Prozesse um. Die Zustände RECOVERY und ESKALATION fassen die jeweils ausgeführten Maßnahmen über beide Plattformen hinweg zusammen.

#vorlage("abb-3-4.png", "Abbildung 3.4: Systemweiter Zustandsautomat für Fehlerbehandlung und Recovery", art: "image")
...
ESKALATION wird erreicht, wenn ein kritisches Fortschrittskriterium verletzt wird oder die lokale Recovery innerhalb ihrer festgelegten Grenze erfolglos bleibt. Ein einzelner Linux- Dienst wird zunächst isoliert neu gestartet und anschließend im Zustand RECOVERY funktional geprüft. Kann die betroffene Ausführungsbasis selbst keine Recovery mehr durchführen, lösen IWDG beziehungsweise Raspberry-Pi-Hardware-Watchdog einen Reset aus; der nächste ausführbare Zustand ist dann BOOT. Damit bildet der Automat das Prinzip der feingranularen Recovery mit kontrollierter Eskalation ab \[2, S. 5 und 8; 6, S. 31–32; 11, S. 16\].

#echtetabelle("Tabelle 3.5: Übergänge des systemweiten Recovery-Zustandsautomaten", (0.4fr, 1.5fr, 4fr), ([ID], [Übergang], [Auslöser beziehungsweise Bedingung],), (
  [T1],
  [BOOT → NORMAL],
